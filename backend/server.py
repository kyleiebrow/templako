...
function showLoading(show, message){let existing=document.querySelector('.loading-overlay');if(existing)existing.remove();if(show){let overlay=document.createElement('div');overlay.className='loading-overlay';overlay.innerHTML='<div class="loading-card"><div class="spinner"></div><p>'+message+'</p></div>';document.body.appendChild(overlay);}}
function hideLoading(){let existing=document.querySelector('.loading-overlay');if(existing)existing.remove();}
...
function handleBack(){
    if(step === 'register' && q > 0){ q--; render(); }
        ...
        if(resetStage > 0){ resetStage--; render(); }
        else{ step = 'login'; render(); }
    }
    else{ window.location.href = '/'; }
}

function toggleEULAAccept() { const checkbox = document.getElementById('eulaCheckbox'); const btn = document.getElementById('acceptEulaBtn'); btn.disabled = !checkbox.checked; }
function showEULA() { document.getElementById('eulaModal').classList.add('show'); }
function closeEULA() { document.getElementById('eulaModal').classList.remove('show'); }
function acceptEULA() { regData.agreedToEula = true; closeEULA(); register(); }
function showErrorModal(message, identifier, password) { document.getElementById('errorMessage').innerHTML = message || 'Invalid credentials.'; document.getElementById('errorModal').classList.add('show'); if(identifier) localStorage.setItem('saved_login_identifier', identifier); if(password) localStorage.setItem('saved_login_password', password); }
function closeErrorModal() { document.getElementById('errorModal').classList.remove('show'); }
function closeErrorModalAndRetry() { closeErrorModal(); const identifierInput = document.getElementById('loginIdentifier'); const passwordInput = document.getElementById('loginPassword'); if(identifierInput) { identifierInput.value = localStorage.getItem('saved_login_identifier') || ''; identifierInput.focus(); } if(passwordInput) passwordInput.value = localStorage.getItem('saved_login_password') || ''; }
...
function generateAutoEmail(name, role){
    ...
    if(role === 'vendor'){ return `${slug}@lako.vendor`; }
    else{ return `${slug}@lako.customer`; }
}

function initLocationMap(){
    if(typeof L === 'undefined'){ setTimeout(initLocationMap, 100); return; }
    if(map) map.remove();
...
function confirmLocation(){
    if(!regData.location.lat || !regData.location.lng){ showToast('Please pin your location on the map first'); return; }
    ...
    if(step === 'login'){
        ...
        if(current === 'Contact method'){
            ...
            if(regData.profilePhoto){
                setTimeout(function(){let preview = document.getElementById('photoPreview'); if(preview) preview.innerHTML = '<div class="file-info"><img class="thumbnail" src="'+regData.profilePhoto+'"><span>'+regData.profilePhotoName+'</span><button onclick="removePhoto(\'photo\')"><i class="fas fa-times"></i></button></div>';},10);
            }
        }
            ...
            if(regData.logo){
                setTimeout(function(){let preview = document.getElementById('logoPreview'); if(preview) preview.innerHTML = '<div class="file-info"><img class="thumbnail" src="'+regData.logo+'"><span>'+regData.logoName+'</span><button onclick="removePhoto(\'logo\')"><i class="fas fa-times"></i></button></div>';},10);
            }
        }
        ...
        if(resetStage === 0){
            ...
            c.innerHTML = `
                <div class="card">
                    ...
                    <div class="input-group">
                        <input type="password" id="confirmPassword" placeholder="Confirm password" oninput="checkResetMatch()">
...
function checkResetStrength(){
    ...
    let percent = strength<=2?25 : strength<=3?50 : strength<=4?75 : 100;
    let color = strength<=2?'#e53935' : strength<=3?'#fb8c00' : strength<=4?'#1e88e5' : '#2d8c3c';
    let text = strength<=2?'Weak' : strength<=3?'Fair' : strength<=4?'Good' : 'Strong';
    
    let fill = document.getElementById('resetStrengthFill');
    let txtEl = document.getElementById('resetStrengthText');
    if(fill){ fill.style.width = percent+'%'; fill.style.background = color; }
    if(txtEl){ txtEl.innerHTML = 'Strength: <span style="color:'+color+'">'+text+'</span>'; }
    checkResetMatch();
}
...
function checkResetMatch(){
    ...
    let msg = document.getElementById('resetMatchMsg');
...
async function sendForgotOTP(){
    ...
    showLoading(true, 'Sending reset code...');
    
    let isPhone = /^\d{10,11}$/.test(identifier);
    let body = isPhone ? { phone: identifier } : { email: identifier };
    ...
    try {
        ...
        if(res.ok && data.success){
            ...
            showToast(data.error || 'Account not found');
        }
    } catch(error) {
        hideLoading();
...
async function register(){
    ...
    if(res.ok && data.requires_verification){
        ...
        let loginEmail = contactMethod === 'phone' ? regData.autoGeneratedEmail : regData.email;
        let loginRes = await fetch('/api/auth/login', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: loginEmail, password: regData.password })
        });
        let loginData = await loginRes.json();
        
        if(loginRes.ok){
            localStorage.setItem('session_token', loginData.session_token);
            localStorage.setItem('user_role', loginData.role || userRole);
            showSuccessToast('Account created! Redirecting...');
            setTimeout(() => {
                if(loginData.role === 'admin') window.location.href = '/admin';
                else if(loginData.role === 'customer') window.location.href = '/customer';
                else if(loginData.role === 'vendor') window.location.href = '/vendor';
                else window.location.href = '/auth';
            }, 1500);
        } else {
            showToast(`Account created! Please login with:\nEmail: ${loginEmail}\nPassword: ${regData.password}`);
            step = 'login';
            savedIdentifier = loginEmail;
            savedPassword = regData.password;
            render();
        }
    } else {
...
async function handleLogin(){
    ...
    let isPhone = /^\d{10,11}$/.test(identifier);
    let body = isPhone ? { phone: identifier, password: password } : { email: identifier, password: password };
    ...
    try {
        ...
        if(res.ok && data.success){
            ...
            else if (data.role === 'customer') window.location.href = '/customer';
            else if (data.role === 'vendor') window.location.href = '/vendor';
            else window.location.href = '/auth';
        } else {
            showErrorModal(data.error || 'Invalid credentials.', identifier, password);
        }
    } catch (error) {
        hideLoading();
...
function getOTP(){ return Array.from(document.querySelectorAll('.otp-input')).map(i => i.value).join(''); }

function startAutoOTP(){
    otpInterval = setInterval(async function(){
        let url = contactMethod === 'phone' ? '/api/auth/check-otp?phone=0'+regData.phone : '/api/auth/check-otp?email='+encodeURIComponent(regData.email);
...
async function verifyOTP(){
    ...
    let body = contactMethod === 'phone' ? {phone: '0' + regData.phone, otp: otp} : {email: regData.email, otp: otp};
...
async function resendOTP(){
    let body = contactMethod === 'phone' ? {phone: '0' + regData.phone} : {email: regData.email};
...
.map-wrapper{position:relative;border-radius:20px;overflow:hidden;margin-bottom:16px}
.map-container{height:400px;width:100%;background:#e8ece8;border-radius:20px;position:relative;z-index:1}
...
.leaflet-popup-content-wrapper{z-index:1000}
.map-controls{position:absolute;bottom:16px;right:16px;display:flex;flex-direction:column;gap:8px;z-index:400}
...
.mt-3{margin-top:12px}
.mb-2{margin-bottom:8px}
.avatar{width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,#3a7b4d,#2e6640);display:flex;align-items:center;justify-content:center;color:white;font-size:20px;object-fit:cover}
...
.comment-delete{color:#e53935;font-size:11px;cursor:pointer}
...
.location-badge{background:#e3f5e3;padding:4px 12px;border-radius:20px;font-size:11px;color:#3a7b4d;display:inline-flex;align-items:center;gap:6px}
.gps-indicator{display:inline-block;width:8px;height:8px;border-radius:50%;background:#3a7b4d;animation:pulse 1.5s infinite}
@keyframes pulse{0%{opacity:1;transform:scale(1)}100%{opacity:0;transform:scale(2)}}
.leaflet-routing-container{display:none}
.route-step{background:#f8faf8;border-radius:12px;padding:12px;margin-bottom:8px;display:flex;align-items:center;gap:12px;cursor:pointer;transition:all 0.2s}
.route-step.completed{opacity:0.6;background:#e3f5e3}
.route-step.active{background:#e3f5e3;border-left:4px solid #3a7b4d}
.route-step-icon{width:32px;height:32px;background:#3a7b4d;border-radius:50%;display:flex;align-items:center;justify-content:center;color:white;font-size:12px}
.route-step.completed .route-step-icon{background:#8da38d}
.route-step-text{flex:1;font-size:13px}
.route-step-distance{font-size:11px;color:#8da38d}
.route-progress{height:4px;background:#e0e8e0;border-radius:2px;margin:12px 0;overflow:hidden}
.route-progress-bar{height:100%;background:#3a7b4d;transition:width 0.3s}
.product-section{margin-bottom:24px}
.product-section-title{font-size:16px;font-weight:700;margin-bottom:12px;color:#2c3e2c}
.otp-input-group{display:flex;gap:8px;justify-content:center;margin:20px 0}
.otp-input{width:50px;height:50px;text-align:center;font-size:24px;font-weight:bold;border:2px solid #e0e8e0;border-radius:12px;background:white}
.otp-input:focus{outline:none;border-color:#3a7b4d}
.verified-badge{color:#3a7b4d;font-size:12px;margin-left:8px}
.vendor-logo{width:40px;height:40px;border-radius:12px;background:#3a7b4d;display:flex;align-items:center;justify-content:center;color:white;font-size:18px;margin-right:12px}
.visit-item{display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #e8ece8}
...
<div class="app-bar">
    <div class="app-bar-left">
        <div>
            ...
            <div class="app-bar-subtitle">GPS Based Vendor Discovery</div>
        </div>
    </div>
...
<div id="hamburgerMenu" class="hamburger-menu">
    ...
    <div class="menu-divider"></div>
    <div class="menu-item" onclick="closeMenu(); openSettingsModal()"><i class="fas fa-user-cog"></i> Account Settings</div>
    <div class="menu-divider"></div>
...
<div id="preferencesModal" class="modal"><div class="modal-content"><div class="modal-header"><h3>Your Preferences</h3><span class="modal-close" onclick="closePreferencesModal()">&times;</span></div><div><strong>Food Categories</strong></div><div id="prefCategories" class="filter-chips" style="flex-wrap:wrap"></div><div class="mt-3"><strong>Max Budget (P)</strong></div><div class="slider-container"><input type="range" id="prefPriceMax" min="0" max="1000" step="50" value="500"><div class="price-value">Up to P<span id="prefPriceMaxVal">500</span></div></div><div class="mt-3"><strong>Max Distance (meters)</strong></div><input type="range" id="prefDistance" min="5" max="200" step="5" value="50" style="width:100%"><div class="flex justify-between"><span>5m</span><span id="prefDistanceVal">50m</span><span>200m</span></div><button class="btn mt-4" onclick="savePreferences()">Save Preferences</button></div></div>
<div id="settingsModal" class="modal"><div class="modal-content"><div class="modal-header"><h3>Account Settings</h3><span class="modal-close" onclick="closeSettingsModal()">&times;</span></div><div style="text-align:center;margin-bottom:16px"><div id="settingsAvatar" class="avatar-lg" onclick="document.getElementById('settingsPhotoInput').click()" style="cursor:pointer;margin:0 auto 16px"><i class="fas fa-camera"></i></div><input type="file" id="settingsPhotoInput" accept="image/*" style="display:none" onchange="previewSettingsPhoto(this)"><button class="btn-outline btn-sm" onclick="document.getElementById('settingsPhotoInput').click()">Change Photo</button></div><input id="settingsFullName" class="input" placeholder="Full Name"><div class="flex gap-2"><input id="settingsEmail" class="input" placeholder="Email Address" type="email" style="flex:1"><button class="btn-outline btn-sm" onclick="sendEmailOTP()" id="verifyEmailBtn">Verify Email</button></div><div id="emailVerifiedStatus" class="text-secondary" style="font-size:12px;margin-top:-8px;margin-bottom:12px"></div><div class="flex gap-2"><input id="settingsPhone" class="input" placeholder="Phone Number" style="flex:1"><button class="btn-outline btn-sm" onclick="sendPhoneOTP()" id="verifyPhoneBtn">Verify Phone</button></div><div id="phoneVerifiedStatus" class="text-secondary" style="font-size:12px;margin-top:-8px;margin-bottom:12px"></div><input id="settingsPassword" class="input" placeholder="New Password" type="password"><input id="settingsConfirmPassword" class="input" placeholder="Confirm Password" type="password"><button class="btn" onclick="saveSettings()">Save Changes</button></div></div>
...
<div id="routingModal" class="modal"><div class="modal-content"><div class="modal-header"><h3>Turn-by-Turn Directions</h3><span class="modal-close" onclick="closeRoutingModal()">&times;</span></div><div id="routingInfo"></div><div class="route-progress"><div class="route-progress-bar" id="routeProgressBar" style="width:0%"></div></div><div id="routeStepsList"></div><div id="routingMap" style="height:250px;width:100%;margin-top:12px;border-radius:16px;overflow:hidden;background:#e8ece8"></div></div></div>

<script>
let sessionToken = localStorage.getItem('session_token');
let userLocation = null, watchId = null;
let allVendors = [], allProducts = [], savedVendors = [], followedVendors = [];
let currentUserId = null, allPosts = [], userProfile = null;
let page = 'map', map = null, markerCluster = null, heatLayer = null, userMarker = null;
let heatActive = true;
let userPreferences = {categories: [], priceMax: 500, maxDistance: 50};
...
async function loadProfile() {
    ...
    if (data) { 
        ...
        userProfile = data;
        if (data.photo) userPhotoData = data.photo;
    }
}
...
async function loadVisits() {
    const data = await api('/api/customer/visits');
    if (data) {
        vendorVisits = data.vendor_visits || [];
        productVisits = (data.product_visits || []).map(pv => {
            let product = allProducts.find(p => p.id == pv.product_id);
            let vendor = allVendors.find(v => v.id == pv.vendor_id);
            return { 
                ...pv, 
...
async function recordVendorVisit(vendorId, vendorName) {
    vendorVisits.unshift({ id: Date.now(), vendor_id: vendorId, vendor_name: vendorName, time: new Date().toISOString() });
    if (vendorVisits.length > 20) vendorVisits.pop();
    await api('/api/customer/record-visit', { method: 'POST', body: JSON.stringify({ type: 'vendor', id: vendorId }) });
}

async function recordProductVisit(productId, productName, vendorId, category) {
    let vendor = allVendors.find(v => v.id == vendorId);
    productVisits.unshift({ 
        id: Date.now(), 
    ...
    if (productVisits.length > 20) productVisits.pop();
    await api('/api/customer/record-visit', { method: 'POST', body: JSON.stringify({ type: 'product', id: productId }) });
}
...
async function loadUserPreferences() { 
    ...
    if (data) { 
        ...
        if (!userPreferences.categories) userPreferences.categories = [];
    } 
}
...
function showMap() {
    ...
    setTimeout(() => {
        if (map) map.remove();
        let centerLat = (userLocation && userLocation.lat) ? userLocation.lat : TIAONG_LAT;
        let centerLng = (userLocation && userLocation.lng) ? userLocation.lng : TIAONG_LNG;
        ...
        if (userLocation && userLocation.lat) {
            ...
            userMarker = L.marker([userLocation.lat, userLocation.lng], { icon: userIcon }).addTo(map).bindPopup('You are here');
        }
        ...
        allVendors.forEach(v => {
            if (v.latitude && v.longitude) {
                ...
                let firstLetter = (v.business_name && v.business_name.charAt(0).toUpperCase()) || 'V';
                ...
                let marker = L.marker([v.latitude, v.longitude], { icon: vendorIcon }).bindPopup(`
                    <div><b>${escapeHtml(v.business_name)}</b><br>${escapeHtml(v.category)}<br><span class="vendor-status ${isOpen?'open':'closed'}">${isOpen?'Open Now':'Closed'}</span></div>
                    <div class="flex gap-2 mt-2">
                        <button class="btn-outline btn-sm" onclick="showVendorModal('${v.id}'); map.closePopup();">View</button>
                        <button class="btn-outline btn-sm" onclick="showRoute(${v.latitude}, ${v.longitude}, '${escapeHtml(v.business_name)}'); map.closePopup();">Directions</button>
                    </div>
                `);
                markerCluster.addLayer(marker);
            }
...
function getRouteSteps(route) {
    ...
    for (let i = 0; i < instructions.length; i++) {
        ...
        let text = inst.text || '';
        let distance = inst.distance || 0;
        steps.push({ text: text, distance: distance, completed: false });
    }
    return steps;
}
...
function showRoute(destLat, destLng, destName) {
    ...
    closeModal();
    document.getElementById('routingInfo').innerHTML = `<div class="route-summary">Loading directions to ${escapeHtml(destName)}...</div>`;
    ...
    setTimeout(() => {
        ...
        currentRouteMap = L.map('routingMap').setView([(userLocation.lat + destLat)/2, (userLocation.lng + destLng)/2], 13);
        L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png').addTo(currentRouteMap);
        
        let startIcon = L.divIcon({ html: '<div style="background:#3a7b4d;width:12px;height:12px;border-radius:50%;border:2px solid white;"></div>', iconSize: [12, 12], className: '' });
        let destIcon = L.divIcon({ html: '<div style="background:#e53935;width:12px;height:12px;border-radius:50%;border:2px solid white;"></div>', iconSize: [12, 12], className: '' });
        
        L.marker([userLocation.lat, userLocation.lng], { icon: startIcon }).bindPopup('Start: Your Location').addTo(currentRouteMap);
        L.marker([destLat, destLng], { icon: destIcon }).bindPopup('Destination: ' + destName).addTo(currentRouteMap);
        
        L.Routing.control({
            waypoints: [L.latLng(userLocation.lat, userLocation.lng), L.latLng(destLat, destLng)],
            routeWhileDragging: false,
            showAlternatives: false,
            lineOptions: { styles: [{ color: '#3a7b4d', weight: 5, opacity: 0.8 }] },
            addWaypoints: false,
            ...
            let estimatedTime = Math.round(route.summary.totalTime / 60);
            
            currentRouteSteps = getRouteSteps(route);
            currentStepIndex = 0;
            
            document.getElementById('routingInfo').innerHTML = `
                <div class="route-summary">
                    <strong>${escapeHtml(destName)}</strong><br>
                    Distance: ${distance} km | Est. time: ${estimatedTime} min
                </div>
            `;
...
function renderRouteSteps() {
    let stepsHtml = '<div style="max-height:300px;overflow-y:auto">';
    currentRouteSteps.forEach((step, idx) => {
        let stepClass = 'route-step';
        if (step.completed) stepClass += ' completed';
        if (idx === currentStepIndex && !step.completed) stepClass += ' active';
        stepsHtml += `
            <div class="${stepClass}" onclick="markStepCompleted(${idx})">
                <div class="route-step-icon">${idx + 1}</div>
                <div class="route-step-text">${escapeHtml(step.text)}</div>
                <div class="route-step-distance">${step.distance > 0 ? (step.distance/1000).toFixed(1) + 'km' : ''}</div>
            </div>
        `;
    });
    stepsHtml += '</div>';
    
    let completedCount = currentRouteSteps.filter(s => s.completed).length;
    let progressPercent = (completedCount / currentRouteSteps.length) * 100;
...
function markStepCompleted(stepIdx) {
    if (stepIdx === currentStepIndex && !currentRouteSteps[stepIdx].completed) {
        currentRouteSteps[stepIdx].completed = true;
        currentStepIndex++;
        renderRouteSteps();
...
function closeRoutingModal() { 
    document.getElementById('routingModal').classList.remove('show'); 
    if (currentRouteMap) {
        currentRouteMap.remove();
        currentRouteMap = null;
    }
...
function showVendors() {
    ...
    document.getElementById('content').innerHTML = `
        <div class="location-badge mb-2"><span class="gps-indicator"></span> ${fallbackUsed ? 'Tiaong, Quezon (Default)' : 'Live GPS Location'}</div>
        <div class="search-bar"><i class="fas fa-search"></i><input type="text" id="vendorSearch" placeholder="Search vendors..." oninput="filterVendorList()"></div>
...
function vendorCardWithActions(v) {
    return `<div class="card"><div class="flex justify-between items-center"><div onclick="showVendorModal('${v.id}')" style="flex:1"><strong>${escapeHtml(v.business_name)}</strong><div class="text-secondary">${escapeHtml(v.category)}</div><div class="stars mt-1">${'★'.repeat(Math.floor(v.rating||0))}</div></div><div class="flex gap-2"><button class="btn-outline btn-sm" onclick="event.stopPropagation(); unsaveVendor('${v.id}')">Unsave</button><button class="btn-outline btn-sm" onclick="event.stopPropagation(); unfollowVendor('${v.id}')">Unfollow</button></div></div></div>`;
}
...
function showSavedVendors() {
    ...
    document.getElementById('content').innerHTML = `<div class="flex justify-between items-center mb-3"><h3>Saved Vendors</h3><button class="btn-outline btn-sm" onclick="showPage('vendors')">Back</button></div>${savedList.map(v => `<div class="card flex justify-between items-center"><div><strong>${escapeHtml(v.business_name)}</strong><div class="text-secondary">${escapeHtml(v.category)}</div></div><button class="btn-outline btn-sm" onclick="unsaveVendor('${v.id}')">Unsave</button></div>`).join('') || '<div class="card text-center">No saved vendors</div>'}`;
}

function showFollowedVendors() {
    let followedList = allVendors.filter(v => followedVendors.some(f => f.id === v.id));
    document.getElementById('content').innerHTML = `<div class="flex justify-between items-center mb-3"><h3>Following Vendors</h3><button class="btn-outline btn-sm" onclick="showPage('vendors')">Back</button></div>${followedList.map(v => `<div class="card flex justify-between items-center"><div><strong>${escapeHtml(v.business_name)}</strong><div class="text-secondary">${escapeHtml(v.category)}</div></div><button class="btn-outline btn-sm" onclick="unfollowVendor('${v.id}')">Unfollow</button></div>`).join('') || '<div class="card text-center">Not following any vendors</div>'}`;
}

function showProducts() {
    let productCategories = [...new Set(allProducts.map(p => p.category).filter(c => c))].sort();
    
    let categoryRecommended = allProducts.filter(p => userPreferences.categories.includes(p.category)).slice(0,6);
    
    let visitedVendorIds = new Set(vendorVisits.map(v => v.vendor_id));
    let visitedProductCategories = new Set();
    
    productVisits.forEach(pv => {
        if (pv.category) visitedProductCategories.add(pv.category);
    });
    
    visitedVendorIds.forEach(vid => {
        let vendorProducts = allProducts.filter(p => p.vendor?.id == vid);
        vendorProducts.forEach(p => {
            if (p.category) visitedProductCategories.add(p.category);
        });
    });
    
    let suggestedProducts = allProducts.filter(p => 
        !userPreferences.categories.includes(p.category) &&
        (visitedVendorIds.has(p.vendor?.id) || visitedProductCategories.has(p.category))
    ).slice(0,6);
    
    let generalProducts = allProducts.filter(p => 
        !userPreferences.categories.includes(p.category) && 
        !suggestedProducts.includes(p)
    );
    ...
    document.getElementById('content').innerHTML = `
        ...
        <div id="recommendedSection" class="product-section"></div>
        <div id="suggestedSection" class="product-section"></div>
        <div id="generalSection" class="product-section"></div>
        <div id="allProductsSection"></div>`;
    
    if (categoryRecommended.length > 0) {
        document.getElementById('recommendedSection').innerHTML = `<div class="product-section-title">Based on Your Preferences</div><div class="product-grid">${categoryRecommended.map(p => productCard(p)).join('')}</div>`;
    }
    if (suggestedProducts.length > 0) {
        document.getElementById('suggestedSection').innerHTML = `<div class="product-section-title mt-3">Suggested For You (Based on Your Visits)</div><div class="product-grid">${suggestedProducts.map(p => productCard(p)).join('')}</div>`;
    }
    if (generalProducts.length > 0) {
        document.getElementById('generalSection').innerHTML = `<div class="product-section-title mt-3">All Products</div><div class="product-grid">${generalProducts.slice(0,12).map(p => productCard(p)).join('')}</div>`;
    }
    filterProductList();
}
...
function filterProductList() {
    ...
    let filtered = allProducts.filter(p => {
        ...
        let matchesBudget = p.price <= userPreferences.priceMax;
        return matchesSearch && matchesCategory && matchesBudget;
    });
    
    let allSection = document.getElementById('allProductsSection');
    if (allSection) {
        if (filtered.length > 0 && (query || activeProductCat !== 'all')) {
            allSection.innerHTML = `<div class="product-section-title mt-3">Search Results</div><div class="product-grid">${filtered.map(p => productCard(p)).join('')}</div>`;
        } else {
...
async function showVendorModal(vendorId) {
    ...
    recordVendorVisit(vendorId, v.business_name);
    ...
    document.getElementById('modalTitle').innerHTML = `<div class="flex items-center gap-2"><div class="vendor-logo">${v.business_name.charAt(0).toUpperCase()}</div> ${escapeHtml(v.business_name)}</div>`;
    document.getElementById('modalBody').innerHTML = `
        <div class="flex gap-2 mb-3">
            <button class="btn-outline btn-sm" onclick="followVendor('${v.id}')">${isFollowed ? 'Unfollow' : 'Follow'}</button>
            <button class="btn-outline btn-sm" onclick="toggleSave('${v.id}')">${isSaved ? 'Unsave' : 'Save'}</button>
            <button class="btn-outline btn-sm" onclick="showRoute(${v.latitude}, ${v.longitude}, '${escapeHtml(v.business_name)}')">Get Directions</button>
        </div>
...
async function loadVendorReviews(vendorId) {
    ...
    if (data?.reviews && data.reviews.length) {
        reviewsDiv.innerHTML = `<strong>Reviews</strong>` + data.reviews.slice(0,3).map(r => `<div class="mt-2 pt-2 border-t"><div class="stars">${'★'.repeat(r.rating)}</div><p class="text-secondary">${escapeHtml(r.comment || '')}</p><small>${new Date(r.created_at).toLocaleDateString()}</small></div>`).join('');
    }
}
...
async function showFeed() {
    ...
    let followedIds = new Set(followedVendors.map(v => v.id));
    let followedPosts = allPosts.filter(p => followedIds.has(p.vendor_id));
    let otherPosts = allPosts.filter(p => !followedIds.has(p.vendor_id) && p.user_id !== currentUserId);
    let sortedPosts = [...followedPosts, ...otherPosts];
    
    document.getElementById('content').innerHTML = `<div class="flex justify-between items-center mb-3"><h3>Community Feed</h3><button class="btn-outline btn-sm" onclick="openPostModal()">Create Post</button></div><div id="feedList"></div>`;
    
    document.getElementById('feedList').innerHTML = sortedPosts.map(p => `
        <div class="card">
            <div class="flex justify-between items-start">
                <div class="flex items-center gap-2">
                    <div class="avatar avatar-sm" onclick="showUserProfile('${p.user_id}')">${!p.user_photo ? '<i class="fas fa-user-circle"></i>' : ''}</div>
                    <div><strong>${p.user_id === currentUserId ? 'You' : escapeHtml(p.author || 'User')}</strong><div class="text-secondary">${timeAgo(p.created_at)}</div></div>
                </div>
                ${p.user_id === currentUserId ? `<button class="btn-outline btn-sm" onclick="deletePost('${p.id}')">Delete</button>` : ''}
            </div>
            <p class="mt-2">${escapeHtml(p.content)}</p>
            <div class="post-actions">
                <button class="post-action-btn ${p.user_liked ? 'liked' : ''}" onclick="likePost('${p.id}', this)"><i class="far fa-heart"></i> ${p.likes || 0}</button>
                <button class="post-action-btn" onclick="toggleComments('${p.id}')"><i class="far fa-comment"></i> ${p.comment_count || 0}</button>
            </div>
...
async function deletePost(postId) {
    if (confirm('Delete this post?')) {
        await api('/api/customer/post/delete', { method: 'POST', body: JSON.stringify({ post_id: postId }) });
...
async function deleteComment(commentId) {
    if (confirm('Delete this comment?')) {
        await api('/api/customer/comment/delete', { method: 'POST', body: JSON.stringify({ comment_id: commentId }) });
    ...
    if (div) {
        div.innerHTML = comments.map(c => `
            <div class="comment-item">
                <div class="flex justify-between items-start">
                    <div>
                        <div class="comment-author" onclick="showUserProfile('${c.user_id}')"><strong>${c.user_id === currentUserId ? 'You' : escapeHtml(c.author || 'User')}</strong></div>
                        <div>${escapeHtml(c.content)}</div>
                        <small>${timeAgo(c.created_at)}</small>
                    </div>
                    ${c.user_id === currentUserId ? `<button class="comment-delete" onclick="deleteComment('${c.id}')">Delete</button>` : ''}
                </div>
            </div>
        `).join('');
        div.innerHTML += `<div class="mt-2"><button class="btn-outline btn-sm" onclick="openReplyModal('${postId}')">Write a reply</button></div>`;
    }
}
...
function showProfile() {
    let followedIds = followedVendors.map(v => v.id);
    let followedCount = followedIds.length;
    let vendorVisitCount = vendorVisits.length;
    let productVisitCount = productVisits.length;
    
    let photoStyle = userPhotoData ? `background-image:url('${userPhotoData}');background-size:cover` : '';
    document.getElementById('content').innerHTML = `
        <div class="card text-center">
            <div class="avatar-lg" style="${photoStyle};margin:0 auto 16px;cursor:pointer" onclick="openSettingsModal()">${!userPhotoData ? '<i class="fas fa-user-circle"></i>' : ''}</div>
            <h3>${escapeHtml(userProfile?.full_name || 'Food Explorer')}</h3>
            <p class="text-secondary">${escapeHtml(userProfile?.email || '')} ${userProfile?.is_email_verified ? '<span class="verified-badge">Verified</span>' : ''}</p>
            <p class="text-secondary">${escapeHtml(userProfile?.phone || 'No phone')} ${userProfile?.is_phone_verified ? '<span class="verified-badge">Verified</span>' : ''}</p>
            <div class="profile-stats">
                <div class="stat-card" onclick="showSavedVendors()"><div class="stat-value">${savedVendors.length}</div><div class="stat-label">Saved</div></div>
                <div class="stat-card" onclick="showFollowedVendors()"><div class="stat-value">${followedCount}</div><div class="stat-label">Following</div></div>
                <div class="stat-card" onclick="showActivityDetails()"><div class="stat-value">${vendorVisitCount + productVisitCount}</div><div class="stat-label">Visits</div></div>
            </div>
            <button class="btn-outline" onclick="openSettingsModal()">Edit Profile</button>
        </div>
        <div class="mt-4"><h3 class="mb-2">Recent Vendor Visits</h3>${vendorVisits.slice(0,5).map(v => `
            <div class="card" onclick="showVendorModal('${v.vendor_id}')">
                <div class="flex justify-between items-center">
                    <div><strong>${escapeHtml(v.vendor_name)}</strong></div>
                    <div class="text-secondary">${timeAgo(v.time)}</div>
                </div>
            </div>
        `).join('') || '<div class="card text-secondary">No vendor visits yet</div>'}</div>
        <div class="mt-4"><h3 class="mb-2">Recent Product Views</h3>${productVisits.slice(0,5).map(p => `
            <div class="card" onclick="showVendorModal('${p.vendor_id}')">
                <div class="flex justify-between items-center">
                    <div><strong>${escapeHtml(p.product_name)}</strong><br><span class="text-secondary">${escapeHtml(p.vendor_name || '')}</span></div>
                    <div class="text-secondary">${timeAgo(p.time)}</div>
                </div>
            </div>
        `).join('') || '<div class="card text-secondary">No product views yet</div>'}</div>`;
}

function showActivityDetails() {
    document.getElementById('content').innerHTML = `
        <div class="flex justify-between items-center mb-3"><h3>My Activity</h3><button class="btn-outline btn-sm" onclick="showPage('profile')">Back</button></div>
        <div class="card"><h4>Vendor Visits (${vendorVisits.length})</h4>${vendorVisits.map(v => `<div class="visit-item"><div><strong>${escapeHtml(v.vendor_name)}</strong></div><div class="visit-time">${timeAgo(v.time)}</div></div>`).join('') || '<div class="text-secondary">No vendor visits</div>'}</div>
        <div class="card mt-3"><h4>Product Views (${productVisits.length})</h4>${productVisits.map(p => `<div class="visit-item"><div><strong>${escapeHtml(p.product_name)}</strong><br><span class="text-secondary">${escapeHtml(p.vendor_name || '')}</span></div><div class="visit-time">${timeAgo(p.time)}</div></div>`).join('') || '<div class="text-secondary">No product views</div>'}</div>
    `;
}

function openSettingsModal() {
    document.getElementById('settingsFullName').value = userProfile?.full_name || '';
    ...
    let emailStatus = document.getElementById('emailVerifiedStatus');
    let phoneStatus = document.getElementById('phoneVerifiedStatus');
    
    if (userProfile?.is_email_verified) {
        emailStatus.innerHTML = '<span class="verified-badge">Email verified</span>';
        document.getElementById('verifyEmailBtn').style.display = 'none';
    } else {
        emailStatus.innerHTML = '<span class="text-secondary">Email not verified</span>';
        document.getElementById('verifyEmailBtn').style.display = 'block';
    }
    
    if (userProfile?.is_phone_verified) {
        phoneStatus.innerHTML = '<span class="verified-badge">Phone verified</span>';
        document.getElementById('verifyPhoneBtn').style.display = 'none';
    } else {
        phoneStatus.innerHTML = '<span class="text-secondary">Phone not verified</span>';
        document.getElementById('verifyPhoneBtn').style.display = 'block';
    }
    
    let avatarDiv = document.getElementById('settingsAvatar');
    if (userPhotoData) {
        avatarDiv.style.backgroundImage = `url('${userPhotoData}')`;
        avatarDiv.style.backgroundSize = 'cover';
        avatarDiv.innerHTML = '';
    } else {
        avatarDiv.style.backgroundImage = '';
        avatarDiv.innerHTML = '<i class="fas fa-camera"></i>';
    }
...
function sendEmailOTP() {
    ...
    api('/api/customer/send-otp', { 
        method: 'POST', 
        body: JSON.stringify({ type: 'email', value: email }) 
    }).then(res => {
        if (res && res.success) {
            pendingVerificationType = 'email';
            pendingVerificationValue = email;
            showOTPModal('email', email);
...
function sendPhoneOTP() {
    let phone = document.getElementById('settingsPhone').value;
    ...
    api('/api/customer/send-otp', { 
        ...
        if (res && res.success) {
            ...
            pendingVerificationValue = phone;
            showOTPModal('phone', phone);
...
function showOTPModal(type, value) {
    let modalHtml = `
        <div id="otpVerificationModal" class="modal show">
            <div class="modal-content">
                <div class="modal-header">
                    <h3>Verify Your ${type === 'email' ? 'Email' : 'Phone'}</h3>
                    <span class="modal-close" onclick="closeOTPModal()">&times;</span>
                </div>
                <div style="text-align:center">
                    <p>We sent a verification code to:</p>
                    <p><strong>${escapeHtml(value)}</strong></p>
                    <div class="otp-input-group" id="otpInputs"></div>
                    <button class="btn" onclick="verifyOTP()">Verify</button>
                    <div style="margin-top:12px">
                        <button class="btn-outline btn-sm" onclick="resendOTP()">Resend Code</button>
                    </div>
    ...
    let existingModal = document.getElementById('otpVerificationModal');
    if (existingModal) existingModal.remove();
    
    document.body.insertAdjacentHTML('beforeend', modalHtml);
    
    let otpInputs = document.getElementById('otpInputs');
    for (let i = 0; i < 6; i++) {
...
async function verifyOTP() {
    ...
    let res = await api('/api/customer/verify-otp', { 
        method: 'POST', 
        body: JSON.stringify({ 
            type: pendingVerificationType, 
            otp: otp,
            value: pendingVerificationValue
        }) 
    });
    
    if (res && res.success) {
        showToast(`${pendingVerificationType === 'email' ? 'Email' : 'Phone'} verified successfully!`);
        closeOTPModal();
        await loadProfile();
...
async function resendOTP() {
    ...
    let res = await api('/api/customer/resend-otp', { 
        method: 'POST', 
        body: JSON.stringify({ 
            type: pendingVerificationType, 
            value: pendingVerificationValue 
    ...
    if (res && res.success) {
        ...
        showToast(res?.error || 'Failed to resend code');
    }
}
...
async function saveSettings() {
    let fullName = document.getElementById('settingsFullName').value;
    let email = document.getElementById('settingsEmail').value;
    let phone = document.getElementById('settingsPhone').value;
    let password = document.getElementById('settingsPassword').value;
    let confirmPassword = document.getElementById('settingsConfirmPassword').value;
    ...
    let res = await api('/api/customer/update-profile', { method: 'POST', body: JSON.stringify(updateData) });
    if (res && res.success) {
        showToast('Profile updated!');
        await loadProfile();
        closeSettingsModal();
...
async function showUserProfile(userId) {
    ...
    if(data){
        let photoStyle = data.photo ? `background-image:url('${data.photo}');background-size:cover` : '';
        document.getElementById('userProfileTitle').innerHTML = escapeHtml(data.full_name || 'User');
        document.getElementById('userProfileBody').innerHTML = `<div class="text-center"><div class="avatar-lg" style="${photoStyle};margin:0 auto 16px">${!data.photo ? '<i class="fas fa-user-circle"></i>' : ''}</div><h3>${escapeHtml(data.full_name || 'User')}</h3><p class="text-secondary">${escapeHtml(data.email || '')}</p></div>`;
...
function togglePrefCategory(cat) { 
    let idx = userPreferences.categories.indexOf(cat); 
    if (idx === -1) userPreferences.categories.push(cat); 
    else userPreferences.categories.splice(idx, 1); 
    showPreferences(); 
}

async function savePreferences() { 
    userPreferences.priceMax = parseInt(document.getElementById('prefPriceMax').value); 
    userPreferences.maxDistance = parseInt(document.getElementById('prefDistance').value); 
    await api('/api/customer/update-preferences', { method: 'POST', body: JSON.stringify(userPreferences) }); 
...
function showAnalytics() {
    let followedCount = followedVendors.length;
    let vendorVisitCount = vendorVisits.length;
    let productVisitCount = productVisits.length;
    document.getElementById('analyticsContent').innerHTML = `<div class="profile-stats"><div class="stat-card"><div class="stat-value">${followedCount}</div><div class="stat-label">Following</div></div><div class="stat-card"><div class="stat-value">${vendorVisitCount}</div><div class="stat-label">Vendor Visits</div></div><div class="stat-card"><div class="stat-value">${productVisitCount}</div><div class="stat-label">Product Views</div></div></div>`;
...
.stats-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-bottom:20px}
.stat-card{background:white;border-radius:20px;padding:16px;text-align:center;border:1px solid #eef2ee}
.stat-value{font-size:28px;font-weight:800;color:#3a7b4d}
.stat-label{font-size:12px;color:#8da38d;margin-top:4px}
.chart-container{background:white;border-radius:20px;padding:16px;margin-bottom:20px;border:1px solid #eef2ee}
.search-bar{background:white;border:1px solid #e0e8e0;border-radius:30px;padding:10px 16px;display:flex;align-items:center;gap:10px;margin-bottom:16px}
.search-bar i{color:#8ba88b}
.search-bar input{flex:1;border:none;background:transparent;font-size:15px;outline:none}
.btn-outline{background:white;border:1px solid #3a7b4d;color:#3a7b4d;padding:6px 14px;border-radius:30px;font-size:12px;cursor:pointer}
.btn{width:100%;padding:12px;background:#3a7b4d;color:white;border:none;border-radius:30px;font-size:14px;font-weight:600;cursor:pointer}
.btn:active{transform:scale(0.97)}
.btn-sm{padding:6px 12px;font-size:12px;width:auto}
.btn-danger{background:#e53935;border:1px solid #e53935;color:white}
.flex{display:flex}
.justify-between{justify-content:space-between}
.items-center{align-items:center}
.gap-2{gap:8px}
...
.text-success{color:#3a7b4d}
.hamburger-menu{position:fixed;top:0;right:-280px;width:280px;height:100vh;background:white;z-index:200;box-shadow:-2px 0 10px rgba(0,0,0,0.1);transition:right 0.3s ease;padding:60px 20px}
.hamburger-menu.show{right:0}
.menu-item{padding:14px;display:flex;align-items:center;gap:12px;cursor:pointer;border-radius:12px}
.menu-item:hover{background:#f0f4f0}
.menu-divider{height:1px;background:#e8ece8;margin:12px 0}
.loading{text-align:center;padding:40px;color:#8da38d}
.toast{position:fixed;bottom:80px;left:20px;right:20px;background:#2c3e2c;color:white;padding:12px;border-radius:30px;text-align:center;z-index:1000;font-size:13px;animation:fadeInUp 0.3s}
@keyframes fadeInUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
.modal{display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:1000;align-items:center;justify-content:center;padding:20px}
.modal.show{display:flex}
.modal-content{background:white;border-radius:24px;max-width:500px;width:100%;max-height:85vh;overflow-y:auto;padding:20px}
.modal-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;font-size:18px;font-weight:700;color:#2c3e2c;padding-bottom:12px;border-bottom:1px solid #e8ece8}
.modal-close{font-size:24px;cursor:pointer;color:#8ba88b;padding:8px}
.input{width:100%;padding:12px 14px;border:1px solid #e0e8e0;border-radius:14px;font-size:14px;margin-bottom:12px;background:#f8faf8}
.input:focus{outline:none;border-color:#3a7b4d}
textarea.input{min-height:80px;resize:vertical}
.map-container{height:300px;width:100%;border-radius:16px;margin-bottom:12px;overflow:hidden;background:#e8ece8}
.hours-row{display:flex;gap:10px;margin-bottom:10px;align-items:center}
.hours-row input{flex:1}
.remove-hour{background:#e53935;color:white;border:none;border-radius:50%;width:30px;height:30px;cursor:pointer;font-size:14px}
.add-hour{background:#3a7b4d;color:white;border:none;border-radius:30px;padding:8px 16px;cursor:pointer;margin-top:8px}
.badge{padding:4px 10px;border-radius:20px;font-size:10px;font-weight:600}
.badge-admin{background:#3a7b4d;color:white}
.badge-vendor{background:#ff9800;color:white}
.badge-customer{background:#8da38d;color:white}
.badge-open{background:#3a7b4d;color:white}
.badge-closed{background:#8da38d;color:white}
.hour-tag{background:#f0f4f0;border-radius:20px;padding:4px 10px;font-size:11px;margin:2px;display:inline-block;color:#2c3e2c}
.vendor-buttons{display:flex;gap:8px;margin-top:12px;padding-top:12px;border-top:1px solid #eef2ee}
.active-dot{width:8px;height:8px;border-radius:50%;display:inline-block;margin-left:6px}
.dot-green{background:#3a7b4d;animation:pulse 1.5s infinite}
.dot-gray{background:#8da38d}
...
.avatar-sm{width:32px;height:32px;font-size:14px}
.close-hamburger{position:absolute;top:20px;right:20px;background:#f0f4f0;border:none;width:36px;height:36px;border-radius:50%;cursor:pointer;font-size:16px;color:#3a7b4d}
.post-images{display:flex;gap:8px;margin-top:8px;flex-wrap:wrap}
.post-image{width:60px;height:60px;border-radius:12px;object-fit:cover}
.comment-item{background:#f8faf8;border-radius:16px;padding:12px;margin-bottom:8px}
...
<div class="app-bar">
    <div class="app-bar-left">
        <div>
            <div class="app-bar-title">Lako Admin</div>
            <div class="app-bar-subtitle">Lako Management Dashboard</div>
        </div>
    </div>
    <div class="app-bar-right">
        <button class="icon-btn" onclick="toggleMenu()"><i class="fas fa-sliders-h"></i></button>
...
<div id="hamburgerMenu" class="hamburger-menu">
    <button class="close-hamburger" onclick="toggleMenu()"><i class="fas fa-times"></i></button>
    <div class="menu-divider"></div>
    <div class="menu-item" onclick="showStats(); toggleMenu()"><i class="fas fa-chart-line"></i> Dashboard</div>
    <div class="menu-item" onclick="showUsers(); toggleMenu()"><i class="fas fa-users"></i> Users</div>
    <div class="menu-item" onclick="showVendors(); toggleMenu()"><i class="fas fa-store"></i> Vendors</div>
    <div class="menu-item" onclick="showPosts(); toggleMenu()"><i class="fas fa-newspaper"></i> Posts & Comments</div>
    <div class="menu-divider"></div>
    <div class="menu-item" onclick="logout()"><i class="fas fa-sign-out-alt"></i> Logout</div>
</div>

<div class="bottom-nav">
    <div class="nav-item" onclick="showStats()"><i class="fas fa-chart-line"></i><span>Stats</span></div>
    ...
    <div class="nav-item" onclick="showPosts()"><i class="fas fa-newspaper"></i><span>Posts</span></div>
</div>
...
<div class="modal" id="viewPostModal">
    <div class="modal-content">
        <div class="modal-header">
            <h3 id="viewPostTitle"><i class="fas fa-newspaper"></i> Post Details</h3>
            <span class="modal-close" onclick="closeViewPostModal()">&times;</span>
        </div>
        <div id="viewPostBody"></div>
    </div>
</div>
...
<!-- Edit Vendor Modal with Map -->
<div class="modal" id="editVendorModal">
    <div class="modal-content">
        <div class="modal-header">
            <h3 id="editVendorTitle"><i class="fas fa-edit"></i> Edit Vendor</h3>
            <span class="modal-close" onclick="closeEditVendorModal()">&times;</span>
        </div>
        <div id="editVendorBody">
            <input type="hidden" id="editVendorId">
            <input type="text" id="editBusinessName" class="input" placeholder="Business Name">
            <input type="text" id="editCategory" class="input" placeholder="Category">
            <textarea id="editAddress" class="input" placeholder="Address"></textarea>
            
            <label style="font-size:13px;color:#2c3e2c;margin-bottom:4px;display:block">📍 Pin Location on Map</label>
            <div class="map-container"><div id="editVendorMap"></div></div>
            <div class="flex gap-2 mb-3">
                <input type="text" id="editLat" class="input" placeholder="Latitude" readonly>
                <input type="text" id="editLng" class="input" placeholder="Longitude" readonly>
            </div>
            
            <input type="text" id="editPhone" class="input" placeholder="Phone Number">
            <input type="email" id="editEmail" class="input" placeholder="Email">
            <textarea id="editDescription" class="input" placeholder="Description"></textarea>
            
            <div class="flex gap-2 mb-3">
                <label class="flex items-center gap-2"><input type="checkbox" id="editIsOpen"> Open Now</label>
            </div>
            
            <h4>Operating Hours</h4>
            <div id="hoursContainer"></div>
            <button type="button" class="add-hour" onclick="addHourField()">+ Add Hours</button>
            
            <div class="flex gap-2 mt-4">
                <button class="btn" onclick="updateVendor()"><i class="fas fa-save"></i> Save Changes</button>
                <button class="btn-outline" onclick="closeEditVendorModal()">Cancel</button>
            </div>
        </div>
    </div>
</div>
...
function getLastActive(user) {
    if (user.last_login) {
        ...
        if (hoursDiff < 24) return '<span class="active-dot dot-green"></span> Active';
        if (hoursDiff < 168) return `<span class="active-dot dot-gray"></span> ${Math.floor(hoursDiff / 24)}d ago`;
        return `<span class="active-dot dot-gray"></span> ${Math.floor(hoursDiff / 168)}w ago`;
    }
    return '<span class="active-dot dot-gray"></span> Inactive';
}
...
async function showStats() {
    ...
    if (!data) {
        document.getElementById('content').innerHTML = '<div class="card text-center text-danger">Failed to load stats</div>';
        return;
    }
    document.getElementById('content').innerHTML = `
        <div class="card" style="background:linear-gradient(135deg,#3a7b4d,#2e6640);color:white;margin-bottom:20px">
            <div class="flex items-center gap-3">
                <div class="avatar" style="background:rgba(255,255,255,0.2)"><i class="fas fa-user-shield"></i></div>
                <div>
                    <p style="opacity:0.9;font-size:13px">Welcome back,</p>
                    <h2 style="font-size:22px">${escapeHtml(adminData?.full_name || 'Admin')}</h2>
                    <p style="opacity:0.9;font-size:12px">Lako Management Dashboard</p>
                </div>
            </div>
        </div>
        ...
        ${data.weekly_labels && data.weekly_labels.length ? `
        <div class="chart-container">
            <canvas id="activityChart"></canvas>
        </div>` : ''}
    `;
    
    if (data.weekly_labels && data.weekly_labels.length && typeof Chart !== 'undefined') {
        setTimeout(() => {
            ...
            if (ctx) {
                new Chart(ctx, {
                    ...
                    data: { 
                        ...
                        datasets: [{ 
                            label: 'Activity', 
                            data: data.weekly_activity || [0,0,0,0,0,0,0], 
                            borderColor: '#3a7b4d', 
                            fill: true,
                            tension: 0.4,
                            backgroundColor: 'rgba(58,123,77,0.1)'
                        }] 
                    },
...
async function showUsers() {
    ...
    if (!data || !data.users) {
        document.getElementById('content').innerHTML = '<div class="card text-center text-danger">Failed to load users</div>';
        return;
    }
    document.getElementById('content').innerHTML = `
        ...
        <div id="usersList">${data.users.map(u => `
            <div class="card" data-email="${(u.email || '').toLowerCase()}" data-name="${(u.full_name || '').toLowerCase()}">
                <div class="flex justify-between items-start">
                    <div class="flex items-center gap-2" style="flex:1">
                        <div class="avatar avatar-sm"><i class="fas fa-user-circle"></i></div>
                        <div>
                            <div class="flex items-center gap-2">
                                <strong>${escapeHtml(u.full_name || 'No name')}</strong>
                                <span class="badge badge-${u.role || 'customer'}">${u.role || 'customer'}</span>
                            </div>
                            <div class="text-secondary" style="font-size:12px">${escapeHtml(u.email || 'No email')}</div>
                            <div class="text-secondary" style="font-size:11px">${u.phone || 'No phone'} • Joined: ${u.created_at ? new Date(u.created_at).toLocaleDateString() : 'Unknown'}</div>
                            <div class="text-secondary" style="font-size:10px;margin-top:2px">${getLastActive(u)}</div>
                        </div>
                    </div>
                    <button class="btn-outline btn-sm btn-danger" onclick="deleteUser('${u.id}')"><i class="fas fa-trash"></i></button>
                </div>
            </div>
...
async function deleteUser(userId) {
    if (confirm('Delete this user permanently? This will delete all their posts and comments.')) {
        const res = await api('/api/admin/user/delete', { method: 'POST', body: JSON.stringify({ user_id: userId }) });
        if (res && res.success) {
...
function filterUsers() {
    ...
    document.querySelectorAll('#usersList .card').forEach(card => {
        ...
        card.style.display = (email.includes(query) || name.includes(query)) ? 'block' : 'none';
    });
}
...
async function showVendors() {
    ...
    if (!data || !data.vendors) {
        document.getElementById('content').innerHTML = '<div class="card text-center text-danger">Failed to load vendors</div>';
        return;
    }
    document.getElementById('content').innerHTML = `
        <div class="search-bar"><i class="fas fa-search"></i><input type="text" id="vendorSearch" placeholder="Search vendors..." oninput="filterVendorList()"></div>
        <div id="vendorsList">${data.vendors.map(v => `
            <div class="card">
                <div class="vendor-info" style="cursor:pointer" onclick="openEditVendorModal('${v.id}')">
                    <div class="flex items-center gap-2">
                        <div class="avatar avatar-sm"><i class="fas fa-store"></i></div>
                        <div>
                            <div class="flex items-center gap-2">
                                <strong>${escapeHtml(v.business_name)}</strong>
                                <span class="badge ${v.is_open ? 'badge-open' : 'badge-closed'}">${v.is_open ? 'Open' : 'Closed'}</span>
                            </div>
                            <div class="text-secondary" style="font-size:12px">${escapeHtml(v.category || 'Uncategorized')} • Rating: ${v.rating || 'New'}</div>
                            <div class="text-secondary" style="font-size:11px">📞 ${v.phone || 'No phone'} • 📍 ${v.address ? v.address.substring(0, 40) : 'No address'}</div>
                        </div>
                    </div>
                    <div class="hour-tags mt-2">${v.operating_hours ? Object.entries(v.operating_hours).map(([day, hours]) => `<span class="hour-tag">${day}: ${hours}</span>`).join('') : '<span class="hour-tag">No hours set</span>'}</div>
                </div>
                <div class="vendor-buttons">
                    <button class="btn-outline btn-sm" onclick="event.stopPropagation(); openEditVendorModal('${v.id}')"><i class="fas fa-map-marker-alt"></i> Edit</button>
                    <button class="btn-outline btn-sm" onclick="event.stopPropagation(); openVendorProducts('${v.id}', '${escapeHtml(v.business_name)}')"><i class="fas fa-utensils"></i> Products</button>
                    <button class="btn-outline btn-sm btn-danger" onclick="event.stopPropagation(); deleteVendor('${v.id}')"><i class="fas fa-trash"></i> Delete</button>
                </div>
            </div>
...
async function showPosts() {
    currentPage = 'posts';
    document.getElementById('content').innerHTML = '<div class="loading"><i class="fas fa-spinner fa-spin"></i> Loading posts...</div>';
    const data = await api('/api/admin/posts');
    if (!data || !data.posts) {
        document.getElementById('content').innerHTML = '<div class="card text-center text-danger">Failed to load posts</div>';
        return;
    }
    allPosts = data.posts;
    document.getElementById('content').innerHTML = `
        <button class="btn mb-3" onclick="openCreatePostModal()"><i class="fas fa-plus"></i> Create Post</button>
        <div class="search-bar"><i class="fas fa-search"></i><input type="text" id="postSearch" placeholder="Search posts..." oninput="filterPosts()"></div>
        <div id="postsList">${allPosts.map(p => `
            <div class="card" data-content="${(p.content || '').toLowerCase()}" data-author="${(p.author || '').toLowerCase()}">
                <div class="flex justify-between items-start">
                    <div class="flex items-center gap-2">
                        <div class="avatar avatar-sm"><i class="fas fa-user-circle"></i></div>
                        <div>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="badge badge-${p.user_role || 'customer'}">${p.user_role || 'customer'}</span>
                            </div>
                            <div class="text-secondary" style="font-size:11px">${timeAgo(p.created_at)}</div>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        <button class="btn-outline btn-sm" onclick="viewPost('${p.id}')"><i class="fas fa-eye"></i> View</button>
                        <button class="btn-outline btn-sm btn-danger" onclick="deletePost('${p.id}')"><i class="fas fa-trash"></i></button>
                    </div>
                </div>
                <p class="mt-2">${escapeHtml(p.content.substring(0, 100))}${p.content.length > 100 ? '...' : ''}</p>
                ${p.images && p.images.length ? `<div class="post-images">${p.images.slice(0,3).map(img => `<img src="${img.thumbnail || img}" class="post-image">`).join('')}</div>` : ''}
                <div class="flex gap-3 mt-3">
                    <span class="text-secondary"><i class="far fa-heart"></i> ${p.likes || 0}</span>
                    <span class="text-secondary"><i class="far fa-comment"></i> ${p.comment_count || 0}</span>
                </div>
            </div>
        `).join('') || '<div class="card text-center text-secondary">No posts found</div>'}</div>`;
    updateActiveNav('posts');
}

function filterPosts() {
    const query = document.getElementById('postSearch')?.value.toLowerCase() || '';
    document.querySelectorAll('#postsList .card').forEach(card => {
        const content = card.dataset.content || '';
        const author = card.dataset.author || '';
        card.style.display = (content.includes(query) || author.includes(query)) ? 'block' : 'none';
    });
}
...
async function viewPost(postId) {
    ...
    const commentsRes = await api(`/api/admin/post/${postId}/comments`);
    const comments = commentsRes?.comments || [];
    
    document.getElementById('viewPostTitle').innerHTML = `<i class="fas fa-newspaper"></i> Post by ${escapeHtml(post.author || 'User')}`;
    document.getElementById('viewPostBody').innerHTML = `
        <div class="card" style="margin-bottom:16px">
            <p>${escapeHtml(post.content)}</p>
            ${post.images && post.images.length ? `<div class="post-images mt-2">${post.images.map(img => `<img src="${img.thumbnail || img}" class="post-image" style="width:80px;height:80px">`).join('')}</div>` : ''}
            <div class="flex gap-3 mt-3">
                <span class="text-secondary"><i class="far fa-heart"></i> ${post.likes || 0} likes</span>
                <span class="text-secondary"><i class="far fa-comment"></i> ${comments.length} comments</span>
            </div>
        </div>
            ...
            ${comments.length ? comments.map(c => `
                <div class="comment-item" data-comment-id="${c.id}">
                    <div class="comment-header">
                        ...
                        <button class="btn-outline btn-sm btn-danger" onclick="deleteComment('${c.id}', '${postId}')"><i class="fas fa-trash"></i></button>
                    </div>
                    <div class="comment-content">${escapeHtml(c.content)}</div>
                </div>
            `).join('') : '<div class="text-secondary text-center" style="padding:20px">No comments yet</div>'}
        </div>
    `;
...
async function deletePost(postId) {
    if (confirm('Delete this post permanently? This will also delete all comments.')) {
        const res = await api('/api/admin/post/delete', { method: 'POST', body: JSON.stringify({ post_id: postId }) });
        if (res && res.success) {
            showToast('Post deleted');
            showPosts();
        } else {
...
async function deleteComment(commentId, postId) {
    if (confirm('Delete this comment?')) {
        const res = await api('/api/admin/comment/delete', { method: 'POST', body: JSON.stringify({ comment_id: commentId }) });
        if (res && res.success) {
            showToast('Comment deleted');
            viewPost(postId);
            showPosts();
...
function openCreatePostModal() {
    ...
    document.getElementById('adminPostImages').value = '';
...
function previewAdminPostImages(input) {
...
async function openEditVendorModal(vendorId) {
    const vendor = await api(`/api/admin/vendors/${vendorId}`);
    if (!vendor || !vendor.vendor) {
        showToast('Failed to load vendor data');
        return;
    }
    
    const v = vendor.vendor;
    document.getElementById('editVendorId').value = v.id;
    document.getElementById('editBusinessName').value = v.business_name || '';
    document.getElementById('editCategory').value = v.category || '';
    document.getElementById('editAddress').value = v.address || '';
    document.getElementById('editLat').value = v.latitude || '';
    document.getElementById('editLng').value = v.longitude || '';
    ...
    document.getElementById('editIsOpen').checked = v.is_open || false;
    
    const hoursRes = await api(`/api/admin/vendors/${vendorId}/hours`);
    const hours = hoursRes?.hours || [];
    const hoursContainer = document.getElementById('hoursContainer');
    if (hours.length) {
        hoursContainer.innerHTML = hours.map((h, idx) => `
            <div class="hours-row">
                <input type="text" placeholder="Day" class="input" value="${escapeHtml(h.day)}" style="margin:0">
                <input type="text" placeholder="Hours" class="input" value="${escapeHtml(h.hours)}" style="margin:0">
                <button type="button" class="remove-hour" onclick="this.parentElement.remove()">×</button>
            </div>
        `).join('');
    } else {
        hoursContainer.innerHTML = '<div class="text-secondary text-center" style="padding:12px">No operating hours set</div>';
    }
    
    document.getElementById('editVendorModal').classList.add('show');
    
    setTimeout(() => {
        initEditVendorMap(v.latitude, v.longitude, v.business_name);
    }, 200);
}

function initEditVendorMap(lat, lng, name) {
    if (typeof L === 'undefined') {
        setTimeout(() => initEditVendorMap(lat, lng, name), 100);
        return;
    }
    
    const defaultLat = lat ? parseFloat(lat) : 14.5995;
    const defaultLng = lng ? parseFloat(lng) : 120.9842;
    
    if (editVendorMap) {
        editVendorMap.remove();
    }
    
    editVendorMap = L.map('editVendorMap').setView([defaultLat, defaultLng], 16);
    L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png', {
        attribution: 'Map data from OpenStreetMap'
    }).addTo(editVendorMap);
    
    const icon = L.divIcon({
        html: '<div style="background:#3a7b4d;width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:white;font-size:16px;border:2px solid white;box-shadow:0 2px 5px rgba(0,0,0,0.2)"><i class="fas fa-store"></i></div>',
        iconSize: [32, 32],
        className: ''
    });
    
    editVendorMarker = L.marker([defaultLat, defaultLng], { icon: icon, draggable: true }).addTo(editVendorMap);
    
    editVendorMarker.on('dragend', function(e) {
        const pos = e.target.getLatLng();
        document.getElementById('editLat').value = pos.lat.toFixed(6);
        document.getElementById('editLng').value = pos.lng.toFixed(6);
    });
    
    editVendorMap.on('click', function(e) {
        editVendorMarker.setLatLng(e.latlng);
...
function addHourField() {
    ...
    if (container.innerHTML.includes('No operating hours set')) {
        container.innerHTML = '';
    }
    const hourDiv = document.createElement('div');
    hourDiv.className = 'hours-row';
    hourDiv.innerHTML = `
        <input type="text" placeholder="Day" class="input" style="margin:0">
        <input type="text" placeholder="Hours" class="input" style="margin:0">
        <button type="button" class="remove-hour" onclick="this.parentElement.remove()">×</button>
    `;
...
async function updateVendor() {
    ...
    const vendorData = {
        ...
        is_open: document.getElementById('editIsOpen').checked
    };
    
    const updateRes = await api(`/api/admin/vendors/${vendorId}`, { method: 'PUT', body: JSON.stringify(vendorData) });
    
    if (updateRes && updateRes.success) {
        const hourRows = document.querySelectorAll('#hoursContainer .hours-row');
        const hours = [];
        hourRows.forEach(row => {
            const inputs = row.querySelectorAll('input');
            if (inputs[0] && inputs[0].value && inputs[1] && inputs[1].value) {
                hours.push({ day: inputs[0].value, hours: inputs[1].value });
            }
        });
        
        await api(`/api/admin/vendors/${vendorId}/hours`, { method: 'PUT', body: JSON.stringify({ hours }) });
...
async function deleteVendor(vendorId) {
    if (confirm('Delete this vendor? This will delete all products, posts, and reviews.')) {
        const res = await api('/api/admin/vendors/' + vendorId, { method: 'DELETE' });
        if (res && res.success) {
...
function filterVendorList() {
    ...
    document.querySelectorAll('#vendorsList .card').forEach(card => {
        const name = card.querySelector('.vendor-info')?.innerText?.toLowerCase() || '';
...
async function openVendorProducts(vendorId, vendorName) {
    ...
    document.getElementById('vendorProductsTitle').innerHTML = `<i class="fas fa-store"></i> ${escapeHtml(vendorName)} - Products`;
    ...
    const response = await api(`/api/admin/vendors/${vendorId}/products`);
    let products = [];
                ...
                <div class="card" style="margin-bottom:8px">
                    <div class="flex justify-between items-start">
                        ...
                        ${p.images && p.images[0] ? 
                            ...
                            `<div style="width:50px;height:50px;background:#f0f4f0;border-radius:8px;display:flex;align-items:center;justify-content:center"><i class="fas fa-utensils"></i></div>`
                        }
                    </div>
...
function closeEditVendorModal() {
    document.getElementById('editVendorModal').classList.remove('show');
    if (editVendorMap) {
        editVendorMap.remove();
        editVendorMap = null;
    }
...
function updateActiveNav(page) {
    document.querySelectorAll('.nav-item').forEach((el, i) => {
        const pages = ['stats', 'users', 'vendors', 'posts'];
...
chartScript.onload = () => {
    if (currentPage === 'stats') setTimeout(showStats, 100);
};
...
def forgot_password():
    ...
    try:
        if email:
            ...
            elif len(phone_digits) == 11 and phone_digits.startswith('09'):
                formatted_phone = phone_digits
            elif len(phone_digits) == 12 and phone_digits.startswith('63'):
                formatted_phone = '0' + phone_digits[2:]
            else:
                formatted_phone = phone_digits
            ...
            supabase.table('users').update({
                'otp_code': otp,
                'otp_expiry': expiry
            }).eq('phone', formatted_phone).execute()
            
            notifications.send_verification_code_sms(formatted_phone, otp, result.data[0].get('full_name'))
            print(f"Reset OTP sent to phone: {formatted_phone} - OTP: {otp}")
...
def verify_reset_otp():
    ...
    try:
        ...
        if not stored_otp:
            return jsonify({'error': 'No OTP requested. Please request a reset code first.'}), 400
        ...
        # OTP is valid - don't clear it yet, keep for final reset
        return jsonify({'success': True})
...
def reset_password():
    ...
    strength = 0
    if len(new_password) >= 6: strength += 1
    if len(new_password) >= 10: strength += 1
    ...
    if strength < 3:
        return jsonify({'error': 'Password too weak. Use at least 8 characters with mixed case, numbers, or symbols.'}), 400
    ...
    try:
        ...
        # Delete any existing sessions (force re-login)
...
def update_profile():
    """Update customer profile - name, email, phone, password, photo"""
    try:
        ...
        if 'email' in data and data['email'] and data['email'] != current_user.get('email'):
...
def customer_send_otp():
    """Send OTP for email or phone verification from profile settings"""
    session_token = request.headers.get('X-Session-Token')
    if not session_token:
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    if not verification_type or not value:
    ...
    try:
        ...
        if verification_type == 'email':
            # Don't send OTP to auto-generated emails
            if value.endswith('@lako.customer') or value.endswith('@lako.vendor'):
                return jsonify({'error': 'Cannot verify auto-generated email'}), 400
            
            # Send OTP via email
            notifications.send_verification_code_email(value, otp, user.get('full_name'))
            
        elif verification_type == 'phone':
            # Format phone number to 09 format
            ...
            # Send OTP via SMS
            notifications.send_verification_code_sms(formatted_phone, otp, user.get('full_name'))
...
def customer_verify_otp():
    """Verify OTP for email or phone from profile settings"""
    session_token = request.headers.get('X-Session-Token')
    if not session_token:
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    if not verification_type or not otp or not value:
        return jsonify({'error': 'Type, OTP, and value required'}), 400
    ...
    try:
        ...
        if verification_type == 'email':
            ...
            if value != user.get('email'):
                # Check if email already taken
                email_check = supabase.table('users').select('id').eq('email', value).execute()
                if email_check.data and len(email_check.data) > 0 and email_check.data[0]['id'] != session['user_id']:
                    return jsonify({'error': 'Email already taken'}), 400
                update_data['email'] = value
        else:  # phone
            update_data['phone_verified'] = True
            # Update phone if changed (format to 09)
            if value != user.get('phone'):
                phone_digits = ''.join(filter(str.isdigit, value))
                if len(phone_digits) == 10 and phone_digits.startswith('9'):
                    formatted_phone = '0' + phone_digits
                elif len(phone_digits) == 11 and phone_digits.startswith('09'):
...
@app.route('/api/customer/resend-otp', methods=['POST'])
def customer_resend_otp():
    """Resend OTP for verification from profile settings"""
    session_token = request.headers.get('X-Session-Token')
    if not session_token:
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    try:
        ...
        if verification_type == 'email':
            if not value.endswith('@lako.customer') and not value.endswith('@lako.vendor'):
                notifications.send_verification_code_email(value, otp, user.get('full_name'))
        else:  # phone
            phone_digits = ''.join(filter(str.isdigit, value))
            if len(phone_digits) == 10 and phone_digits.startswith('9'):
...
def admin_api_get_vendors():
    ...
    return jsonify({'vendors': result.data})

@app.route('/api/admin/vendors/<vendor_id>', methods=['GET'])
def admin_api_get_vendor(vendor_id):
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    if not result.data:
        return jsonify({'error': 'Vendor not found'}), 404
    return jsonify({'vendor': result.data[0]})

@app.route('/api/admin/vendors', methods=['POST'])
def admin_api_create_vendor():
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    return jsonify({'success': True, 'vendor': result.data[0] if result.data else None})

@app.route('/api/admin/vendors/<vendor_id>', methods=['PUT'])
def admin_api_update_vendor(vendor_id):
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
...
@app.route('/api/admin/vendors/<vendor_id>', methods=['DELETE'])
def admin_api_delete_vendor(vendor_id):
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
...
@app.route('/api/admin/vendors/<vendor_id>/products', methods=['GET'])
def admin_api_get_vendor_products(vendor_id):
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
...
# ============================================
# VENDOR TOGGLE (Enable/Disable)
# ============================================

@app.route('/api/admin/vendor/toggle-status', methods=['POST'])
def admin_api_toggle_vendor_status():
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    supabase.table('vendors').update({'is_active': active}).eq('id', vendor_id).execute()
...
def admin_api_get_users():
    ...
    result = supabase.table('users').select('id, full_name, email, phone, role, created_at, is_email_verified, is_phone_verified, is_suspended').order('created_at', desc=True).execute()
...
def admin_api_delete_user():
    ...
    for post in (user_posts.data or []):
        supabase.table('comments').delete().eq('post_id', post['id']).execute()
        supabase.table('post_likes').delete().eq('post_id', post['id']).execute()
    ...
    vendor = supabase.table('vendors').select('id').eq('user_id', user_id).execute()
    if vendor.data:
        supabase.table('products').delete().eq('vendor_id', vendor.data[0]['id']).execute()
        supabase.table('vendor_hours').delete().eq('vendor_id', vendor.data[0]['id']).execute()
        supabase.table('vendors').delete().eq('id', vendor.data[0]['id']).execute()
...
def admin_api_suspend_user():
    ...
    supabase.table('users').update({'is_suspended': suspend}).eq('id', user_id).execute()
    
    return jsonify({'success': True})

# ============================================
# POST AND COMMENT MANAGEMENT ENDPOINTS (FIXED)
# ============================================

def update_post_comment_count(post_id):
    """Helper function to update comment count on a post"""
    try:
        comment_count = supabase.table('comments').select('id', count='exact').eq('post_id', post_id).execute()
        supabase.table('posts').update({'comment_count': comment_count.count}).eq('id', post_id).execute()
        return True
    except Exception as e:
...
def admin_api_get_all_posts():
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
    
    result = supabase.table('posts').select('*, users!posts_user_id_fkey(full_name, email, role)').order('created_at', desc=True).execute()
    ...
    for p in result.data:
        ...
        posts.append({
            ...
            'images': p.get('images', []),
            'author': user_data.get('full_name', 'User'),
...
@app.route('/api/admin/post/<post_id>/comments', methods=['GET'])
def admin_api_get_post_comments(post_id):
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
    
    # Fix: Use order with column name and desc=False for ascending
    result = supabase.table('comments').select('*, users!comments_user_id_fkey(full_name, email)').eq('post_id', post_id).order('created_at', desc=False).execute()
...
@app.route('/api/admin/comment/delete', methods=['POST'])
def admin_api_delete_comment():
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    comment = supabase.table('comments').select('post_id').eq('id', comment_id).execute()
    post_id = comment.data[0].get('post_id') if comment.data else None
...
@app.route('/api/admin/post/create', methods=['POST'])
def admin_api_create_post():
    ...
    if result.data:
        return jsonify({'success': True, 'post_id': result.data[0]['id']})
    return jsonify({'error': 'Failed to create post'}), 500

@app.route('/api/admin/post/<post_id>', methods=['GET'])
def admin_api_get_post(post_id):
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
    
    result = supabase.table('posts').select('*, users!posts_user_id_fkey(full_name, email, role)').eq('id', post_id).execute()
...
@app.route('/api/admin/comment/create', methods=['POST'])
def admin_api_create_comment():
    from datetime import datetime, timezone
    ...
    if result.data:
        ...
        update_post_comment_count(post_id)
        return jsonify({'success': True, 'comment_id': result.data[0]['id']})
    
    return jsonify({'error': 'Failed to create comment'}), 500

@app.route('/api/admin/posts/delete-by-user', methods=['POST'])
def admin_api_delete_posts_by_user():
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
    ...
    return jsonify({'success': True, 'deleted_count': len(user_posts.data)})
...
def admin_api_get_analytics():
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
    
    # Get counts
    vendors = supabase.table('vendors').select('*', count='exact').execute()
    products = supabase.table('products').select('*', count='exact').execute()
    users = supabase.table('users').select('*', count='exact').execute()
    posts = supabase.table('posts').select('*', count='exact').execute()
    reviews = supabase.table('reviews').select('*', count='exact').execute()
    
    # Get revenue data
    products_data = supabase.table('products').select('price').execute()
    total_revenue = sum(p.get('price', 0) for p in (products_data.data or []))
    ...
    for i in range(6, -1, -1):
        ...
        day_activity = supabase.table('posts').select('*', count='exact').gte('created_at', day_start).lt('created_at', day_end).execute()
        weekly_activity.append(day_activity.count or 0)
...
def admin_api_get_profile():
    session = require_session(request.headers.get('X-Session-Token'))
    if not session or session['role'] != 'admin':
        return jsonify({'error': 'Unauthorized'}), 401
    
    user = supabase.table('users').select('full_name, email').eq('id', session['user_id']).execute()
    if user.data:
